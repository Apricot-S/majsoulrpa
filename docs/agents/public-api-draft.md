...
扱いが曖昧になります。初期 API では禁止し、必要が出たらユーザー側で 1 つの
...
`set_ready()` は ready、`set_ready(ready=False)` は ready 解除を表す。目標状態を設定する
idempotent な API であることを名前で示すため、`cancel()` / `cancel_ready()` や `ready()` の
alias は追加しない。対局開始も操作対象を明示する `start_match()` を維持し、`start()` の
alias は追加しない。
