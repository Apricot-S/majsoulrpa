...
- [ ] `NoTileEvent` は流局確認templateを1回clickする
- [ ] 流し満貫でも最初に通常の荒牌平局と同じ流局確認templateを1回clickする
- [ ] 流し満貫では流局確認後に達成者ごとの流し満貫演出確認画面を和了演出と同じ方法で処理する
- [ ] 複数人の流し満貫では `NoTileScore.seat is not None` の要素数だけ和了確認templateをclickする
- [ ] `LiujuEvent` は通常流局と同じ流局確認templateを1回clickする
